function Test-SchemaVersion {
    ...
    if (-not $PSBoundParameters.ContainsKey('CurrentVersion')) {
        if ($script:OSDCloudConfig -and $script:OSDCloudConfig.SchemaVersion) {
            $CurrentVersion = $script:OSDCloudConfig.SchemaVersion
        } else {
            $CurrentVersion = "1.0"
        }
    }
