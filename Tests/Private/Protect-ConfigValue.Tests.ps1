# File: Tests/Private/Protect-ConfigValue.Tests.ps1
# Requires -Modules Pester

$here = Split-Path -Parent $MyInvocation.MyCommand.Path
$moduleRoot = Resolve-Path "$here/../../"
. "$moduleRoot/Private/Protect-ConfigValue.ps1"
...
Describe 'Protect-ConfigValue' {
    ...
    Context 'Execution with mocked internals' {
        It 'Should run without throwing using default logic' {
            ...
            { Protect-ConfigValue } | Should -Not -Throw
        }

        It 'Should handle expected failure scenario gracefully' {
            Mock -CommandName Test-Path { $false }
            { Protect-ConfigValue } | Should -Throw
        }
    }
}
