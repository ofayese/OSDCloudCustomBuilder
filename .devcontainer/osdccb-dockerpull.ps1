...
foreach ($image in $dockerImages) {
    ...
    $isolationFlag = if (($env:OS -eq 'Windows_NT' -or [System.Environment]::OSVersion.Platform -eq 'Win32NT') -and (Test-WindowsContainer -Image $image)) {
        "--isolation=process"
    } elseif (($env:OS -eq 'Windows_NT') -and (-not (Test-WindowsContainer -Image $image))) {
        "--isolation=default"
    } else {
        ""
    }
    ...
    try {
        ...
        $dockerRunCmd = "docker run -d " +
            ...
            "--mount type=volume,source=$volumeName,target=$containerMountPath "

        if ($isolationFlag) {
            $dockerRunCmd += "$isolationFlag "
        }

        $dockerRunCmd += "$image $idleCommand"
